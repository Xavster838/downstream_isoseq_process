...
bed12 = pd.read_csv(canonical_bed12_path, sep = "\t", usecols=list(range(12)), names = bed_12_cols)
...
with open(isoform_gff_path, 'r') as file:
    for line in file:
        ...
        attributes = [element.strip() for element in attributes if element]
        attribute_dict = {key_value.split('=')[0]: key_value.split('=')[1] for key_value in attributes}
        if columns[2] == "transcript":
           # if "nbis" in attribute_dict["ID"]: #not sure why, but sometimes gff has some extra annotations with an nbis- name in ID...
           #     continue
...
with open(snakemake.output["keep_isos_lst"], "w") as file:
    for element in list(keep_isos.index):
        file.write(element + '\n')
